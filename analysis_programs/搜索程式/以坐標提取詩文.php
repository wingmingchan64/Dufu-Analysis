...
php h:\github\Dufu-Analysis\analysis_programs\搜索程式\以坐標提取詩文.php 0003:1.2.1
...
3. 如果頁碼有效，顯示詩題
4. 如果是詩組，坐標中第二組數字必須是首碼，後頭必須有冒號：〚0013:1:〛
5. 如果是詩組，坐標中第三組數字必須是行碼：〚0013:1:6〛
6. 如果不是詩組，坐標中第二組數字必須是行碼：〚0003:4〛
7. 行碼可以用範圍數字：〚0003:4-5〛〚0013:1:5-7〛
8. 如果是詩組，坐標中第四組數字必須是句碼；句碼必須是 1 或者 2：〚0013:1:6:1〛
9. 如果不是詩組，坐標中第三組數字必須是句碼；句碼必須是 1 或者 2：〚0003:4.2〛
10. 如果坐標中沒有句碼，行碼後不能有點號（.）
11. 有句碼的坐標，行碼不能用範圍數字
12. 句碼不能用範圍數字
13. 如果是詩組，坐標中第五組數字必須是字碼：〚0013:1:6:1:5〛
15. 如果不是詩組，坐標中第四組數字必須是字碼：〚0003:4:1:2〛
16. 字碼可以用範圍數字
17. 整個坐標中，只能有一組範圍數字
18. 一首詩的首碼、行碼，兩者之間其實沒有關係。行碼是完全獨立的。詩題的行碼一定是 1。
*/
...
require_once( 詩集文件夾 . $result[ 坐標頁碼 ] . 程式後綴 );
require_once( 詩組_詩題 );
